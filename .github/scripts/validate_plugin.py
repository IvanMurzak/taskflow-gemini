...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate a local Antigravity plugin.")
...
def load_json_object(path: Path, errors: list[str]) -> dict[str, Any] | None:
    ...
    try:
        ...
        errors.append("unable to read `plugin.json`")
        return None
    except json.JSONDecodeError:
        errors.append("`plugin.json` must be valid JSON")
        return None
